---
title: "EGZAMIN_WSTEPNY_17_KACPER_FIGURA_155173"
...
Wątek Przewodniczącego (PID wątku = 0) pełni rolę orkiestratora dla danej instancji komisji. Po zainicjowaniu (pobraniu ID i zwolnieniu argumentów), rozpoczyna on cykliczną pracę w pętli `while(1)`.
...
* Operacja jest logowana w systemie, co zapewnia pełną audytowalność procesu (można zweryfikować, który wątek wystawił którą ocenę).
...
## 6. Implementacja wymaganych konstrukcji systemowych (Linki do GitHub)
...
**Repozytorium projektu:** [https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne](https://www.google.com/search?q=https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne)
...
* **`pthread_create()`, `pthread_join()**` – Utworzenie puli wątków egzaminatorów i oczekiwanie na ich zakończenie.
* [komisja.c - Inicjalizacja i sprzątanie wątków](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/komisja.c%23Lxx-Lxx)


* **`pthread_mutex_lock()`, `pthread_mutex_unlock()**` – Ochrona sekcji krytycznych (dostęp do danych stolika i rekrutacja wątków).
* [komisja.c - Sekcja krytyczna wyboru stanowiska](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/komisja.c%23Lxx-Lxx)


* **`pthread_cond_wait()`, `pthread_cond_signal()`, `pthread_cond_broadcast()**` – Zarządzanie pulą wolnych egzaminatorów i sygnalizacja IPC.
* [komisja.c - Oczekiwanie wątku na przydział pracy](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/komisja.c%23Lxx-Lxx)


* **`pthread_cond_timedwait()`** – Oczekiwanie na odpowiedź kandydata z mechanizmem **timeoutu**.
* [komisja.c - Implementacja timeoutu egzaminacyjnego](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/komisja.c%23Lxx-Lxx)


* **`pthread_barrier_wait()`** – Synchronizacja grupowa (zbiórka członków komisji przy stoliku).
* [komisja.c - Bariera startowa przed zadaniem pytania](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/komisja.c%23Lxx)
...
* **`sigaction()`** – Rejestracja zaawansowanych handlerów dla `SIGCHLD` (z flagą `SA_NOCLDSTOP`) i `SIGINT`.
* [dziekan.c - Konfiguracja sigaction w main](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/dziekan.c%23Lxx-Lxx)


* **`kill()`** – Wysłanie sygnału kończącego (`SIGTERM`) do grupy procesów (Ewakuacja/Awaria).
* [dziekan.c - Funkcja handler_ewakuacji](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/dziekan.c%23Lxx-Lxx)



### e. Synchronizacja procesów (Semafory POSIX)

Do sterowania przepływem kandydatów między instancjami (Dziekan -> A -> B) wykorzystano semafory nazwane.

* **`sem_open()`, `sem_close()`, `sem_unlink()**` – Cykl życia semaforów nazwanych (kolejki i miejsca).
* [dziekan.c - Inicjalizacja i czyszczenie semaforów](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/dziekan.c%23Lxx-Lxx)


* **`sem_wait()`** – Blokowanie procesu Komisji w oczekiwaniu na kandydata (Konsument).
* [komisja.c - Oczekiwanie Przewodniczącego na kolejce](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/komisja.c%23Lxx)


* **`sem_post()`** – Sygnalizowanie dostępności kandydata przez Dziekana (Producent).
* [dziekan.c - Wpuszczenie kandydata do systemu](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/dziekan.c%23Lxx)
...
* **`mkfifo()`** – Utworzenie pliku specjalnego kolejki FIFO.
* [dziekan.c - Utworzenie łącza nazwanego](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/dziekan.c%23Lxx)



### g. Segmenty pamięci dzielonej (POSIX SHM)

Główny magazyn danych (tablica studentów) współdzielony między wszystkimi procesami symulacji.

* **`shm_open()`, `ftruncate()`, `mmap()**` – Utworzenie, nadanie rozmiaru i zmapowanie obiektu pamięci.
* [dziekan.c - Pełna inicjalizacja Shared Memory](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/dziekan.c%23Lxx-Lxx)


* **`shm_unlink()`** – Usunięcie obiektu pamięci z systemu po zakończeniu symulacji.
* [dziekan.c - Sprzątanie zasobów SHM](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/dziekan.c%23Lxx)


* **Atrybuty `PTHREAD_PROCESS_SHARED**` – Konfiguracja mutexów i zmiennych warunkowych do pracy w pamięci dzielonej (między procesami).
* [dziekan.c - Inicjalizacja atrybutów synchronizacji IPC](https://github.com/Kazurek11/Egzamin-Wstepny-Systemy-Operacyjne/blob/main/dziekan.c%23Lxx-Lxx)