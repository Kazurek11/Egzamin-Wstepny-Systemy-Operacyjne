...
int main() {
    ...
    sem_unlink(SEM_LICZNIK_KONCA); // Reset licznika końcowego
    ...
    // Nowy semafor zliczający zakończone "sprawy"
    ...
    while (odczytani < faktyczna_liczba_kandydatow) {
        ...
        if (idx < MAX_KANDYDATOW) {
            ...
            if (buf.zdana_matura == 1) {
                ...
                // WAŻNE: Kandydat odrzucony = zakończona sprawa. Podbijamy licznik.
    ...
    // Zamiast wait(NULL), czekamy na semaforze tyle razy, ile jest kandydatów.
    // Dzięki temu mamy pewność, że logika egzaminacyjna dobiegła końca.
    for (int i = 0; i < faktyczna_liczba_kandydatow; i++) {
        sem_wait(sem_licznik_konca);
    }
    // -------------------------------------
    
    dodaj_do_loggera(plik_logu, "[Dziekan] [PID: %d] Wszyscy kandydaci zakończyli proces (sukces/porażka). Zamykam komisje.\n", (int)getpid());
    
    // Teraz bezpiecznie zamykamy komisje, bo wiemy, że nikt już nie zdaje
    for (int i = 0; i < LICZBA_KOMISJI; i++) {
        kill(komisje_pids[i], SIGTERM);
    }
    
    // Sprzątamy ewentualne procesy zombie (kandydaci i komisje)
    while(wait(NULL) > 0);
